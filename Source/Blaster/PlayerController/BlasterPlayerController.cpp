...
bool ABlasterPlayerController::ServerSendChatMessage_Validate(const FString& Message)
{
	// Basic validation - limit message length
...
void ABlasterPlayerController::ServerSendChatMessage_Implementation(const FString& Message)
{
	...
	{
		// Get all player controllers and send message to each
		for (FConstPlayerControllerIterator It = GetWorld()->GetPlayerControllerIterator(); It; ++It)
		{
			if (ABlasterPlayerController* PC = Cast<ABlasterPlayerController>(*It))
			{
				PC->ClientReceiveMessage(PS->GetPlayerName(), Message);
			}
		}
	}
}

void ABlasterPlayerController::ClientReceiveMessage_Implementation(const FString& SenderName, const FString& Message)
{
	if (ABlasterHUD* HUD = Cast<ABlasterHUD>(GetHUD()))
	{
		BlasterHUD->AddChatAnnouncement(SenderName, Message);
		//if (HUD->CharacterOverlay && HUD->CharacterOverlay->ChatInputTextBox)
		//{
		//	// Append new message to existing text
		//	FString CurrentText = HUD->CharacterOverlay->ChatInputTextBox->GetText().ToString();
		//	FString NewText = FString::Printf(TEXT("%s\n%s: %s"), *CurrentText, *SenderName, *Message);
		//	HUD->CharacterOverlay->ChatInputTextBox->SetText(FText::FromString(NewText));
		//}
	}
}
